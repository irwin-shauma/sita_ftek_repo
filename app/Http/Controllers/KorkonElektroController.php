...
class KorkonElektroController extends Controller
{
    //----------------------------------------------------------------------
    // Fungsi untuk permission Korkon Elektro
    //----------------------------------------------------------------------
    public function korkon_elektro_verif_kolokium_awal()
    {
        $lists_id = Kolokium_awal::whereColumn('check_korkon', '<', 'check_mhs_send')
            ->where('tipe_korkon', 'korkon_elektro')
            ->pluck('id');
        if ($lists_id->count() != 0) {
            $lists_array = $lists_id->toArray();
            $lists = Mahasiswa::whereIN('id', $lists_array)->get();
        } else {
            return view('dosen.korkon_elektro.verif_kolokium_awal', compact(
                'lists',
    ...
    public function korkon_elektro_verif_kolokium_awal_download($id)
    {
    ...
    public function korkon_elektro_verif_kolokium_awal_verified(VerifKolokiumAwalRequest $request)
    {
    ...
    public function korkon_elektro_validasi_revisi_awal()
    {
        //Untuk mengecek apakah ada proposal yang sudah direview reviewer.
        // Ini nanti tulis di notepad ya caranya!
        $mahasiswa_sesuai_korkon = Kolokium_awal::where('tipe_korkon', 'korkon_elektro')
        ...
        // dd('test');
        // dd($mahasiswa_sesuai_korkon_array);
        ...
        return view('dosen.korkon_elektro.validasi_revisi_awal', compact('data_mhs', 'data_proposal_seen', 'data_review1', 'data_review2'));
    }

    public function korkon_elektro_validasi_revisi_awal_download(Request $request)
    {
    ...
    public function korkon_elektro_validasi_revisi_awal_validated(Request $request)
    {
        $data_all = $request->all();
        ...
        $progress = Mahasiswa::where('id', $data_all['id_mhs'])
            ->update(['progress' => 2]);
        $progress = Proposal_awal::where('mhs_id', $data_all['id_mhs'])
            ->update(['check_mhs_send' => 3]);
        $progress = Kolokium_awal::where('mhs_id', $data_all['id_mhs'])
            ->update(['check_korkon' => 3]);
    ...
    //------------------------------------------------------------------------------------------
    public function korkon_elektro_verif_kolokium_lanjut()
    {
        ...
        if ($lists_id->count() != 0) {
            ...
            return view('dosen.korkon_elektro.verif_kolokium_lanjut', compact(
                'lists',
    ...
    public function korkon_elektro_verif_kolokium_lanjut_download($id)
    {
        // dd($id);
    ...
    public function korkon_elektro_verif_kolokium_lanjut_verified(VerifKolokiumLanjutRequest $request)
    {
        //$id berisi nim mhs.
        $validated = $request->validated();
        if ($validated['check_korkon'] < 1 || $validated['check_korkon'] == null) {
    ...
    public function korkon_elektro_validasi_revisi_lanjut()
    {
        $mahasiswa_sesuai_korkon = Kolokium_lanjut::where('tipe_korkon', 'korkon_elektro')
            ->where('check_korkon', 2)
            ->pluck('mhs_id');
        if ($mahasiswa_sesuai_korkon->count() != 0) {
            $mahasiswa_sesuai_korkon_array = $mahasiswa_sesuai_korkon->toArray();
        ...
        return view('dosen.korkon_elektro.validasi_revisi_lanjut', compact('data_mhs', 'data_proposal_seen', 'data_review1', 'data_review2'));
    }

    public function korkon_elektro_validasi_revisi_lanjut_download(Request $request)
    {
        $data_all = $request->all();
        $id = $data_all['nim'];
        if ($data_all['action'] == 'mahasiswa') {
    ...
    public function korkon_elektro_validasi_revisi_lanjut_validated(Request $request)
    {
        $data_all = $request->all();
        ...
        // $progress = Mahasiswa::where('id', $data_all['id_mhs'])
        // ->update(['progress' => 2]);
        $progress = Proposal_lanjut::where('mhs_id', $data_all['id_mhs'])
            ->update(['check_mhs_send' => 3]);
        $progress = Kolokium_lanjut::where('mhs_id', $data_all['id_mhs'])
            ->update(['check_korkon' => 3]);
    ...
    //-------------------------------------------------------------------------------------------------------------
    public function korkon_elektro_penjadwalan_awal()
    {
        $current = Carbon::now();
        $dosen = Dosen::all();
        // $id_reviewed = Reviewer::where('jenis_review', 'kolokium_awal')->pluck('mhs_id');
        // if ($id_reviewed->count() != 0) {
        //     $id_reviewed_array = $id_reviewed->toArray();
        // } else {
        //     $id_reviewed_array = [];
        // }
        ...
        $lists = Kolokium_awal::where('check_korkon', 1)
            ...
            // ->whereNotIn('mhs_id', $id_reviewed_array)
            ->pluck('mhs_id');
        // dd($lists);
        if ($lists->count() != 0) {
        ...
        return view('dosen.korkon_elektro.penjadwalan_awal', compact('lists', 'nama', 'mhs', 'dosen', 'current', 'lists_dosen'));
    }

    public function korkon_elektro_penjadwalan_awal_data(PenjadwalanAwalRequest $request)
    {
        $current = Carbon::now();
        $validated = $request->validated();
        // dd($validated);
    ...
    //-------------------------------------------------------------------------------------------------------------
    public function korkon_elektro_penjadwalan_lanjut()
    {
        ...
        $current = Carbon::now();
        $dosen = Dosen::all();
        // $id_finished = Reviewer::where('jenis_review', 'kolokium_lanjut')->pluck('mhs_id');
        // if ($id_finished->count() != 0) {
        //     $id_finished = $id_finished->toArray();
        // } else {
        //     $id_finished = [];
        // }
        // $id_reviewed = Reviewer::where('jenis_review', 'kolokium_awal')
        //     ->whereNotIN('mhs_id', $id_finished)
        //     ->pluck('mhs_id');
        // if ($id_reviewed->count() != 0) {
        //     $id_reviewed = $id_reviewed->toArray();
        // } else {
        //     return view('dosen.korkon_elektro.penjadwalan_lanjut', compact('lists', 'nama', 'mhs', 'dosen', 'current'));
        // }
        // $lists = Kolokium_lanjut::whereIN('mhs_id', $id_reviewed)
        ...
        return view('dosen.korkon_elektro.penjadwalan_lanjut', compact('lists', 'nama', 'mhs', 'dosen', 'current', 'reviewer1', 'reviewer2'));
    }

    public function korkon_elektro_penjadwalan_lanjut_data(PenjadwalanLanjutRequest $request)
    {
        $current = Carbon::now();
        $validated = $request->validated();
        $mhs = Mahasiswa::where('id', $validated['id_mhs'])->get();
