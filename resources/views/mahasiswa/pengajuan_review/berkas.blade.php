...
@section('title', 'Berkas Pengajuan Review')
@section('content')
...
<h6>Tiap file, Max: 10MB; Bertipe .jpg, .png, .pdf. Khusus untuk makalah/paper pengajuan review adalah : .docx</h6>

<form action="{{ route('pengajuan_review_berkas_upload') }}" method="POST" enctype="multipart/form-data">
    @csrf
            ...
            <div class="col-sm-6">
                ...
                <div class="modal fade" id="modal_simpan">
                    ...
                    <div class="modal-content bg-white">
                        ...
                        Apakah anda yakin ingin mengupload berkas Pengajuan Review? 
                        <br>
...
@elseif($progress > 4)
    {{-- @if ($progress === 5)
        <p>Anda sudah mengupload berkas pengajuan review, silahkan menunggu admin memverifikasi berkas anda</p>
    @else
        <p>Selamat, Anda telah menyelesaikan skripsi anda!.</p>
    @endif --}}

    @if ($check_admin > 0 && ($check_admin == $pengajuan_review_status[0]->check_mhs_send))
    <h6>Tiap file, Max: 10MB; Bertipe .jpg, .png, .pdf. Khusus untuk makalah/paper pengajuan review adalah : .docx</h6>
        ...
        <div class="content">
            <div class="row">
                ...
                <div class="col-sm-6">
                    ...
                    <input type="hidden" class="form-control" name="{{ $value }}" value={{ $pengajuan_review_status[0]->$value }}>
                    @if ($pengajuan_review_status[0]->$value == 0)
                    ...
                    <div class="modal fade" id="modal_simpan">
                        ...
                        <div class="modal-content bg-white">
                            ...
                            <div class="modal-body">
                            Apakah anda yakin ingin mengupload berkas Pengajuan Review yang tidak terverifikasi? 
                            <br>
                            (Data yang tersimpan tidak bisa diubah lagi)
                            </div>
    ...
    @elseif($check_admin == 0 && ($check_admin < $pengajuan_review_status[0]->check_mhs_send))
        <p>Anda sudah mengupload berkas pengajuan review, silahkan menunggu admin memverifikasi berkas anda</p>
    @elseif($check_admin > 0 && ($check_admin < $pengajuan_review_status[0]->check_mhs_send))
        <p>Anda sudah mengupload berkas pengajuan review yang tidak terverifikasi, silahkan menunggu admin memverifikasi berkas anda</p>
    @endif
    ...
    <tbody>
        ...
        <tr>
            ...
            @if ($check_admin == 0 || $check_admin == null)
                @if ($pengajuan_review_status[0]->$key == null)
                    Belum Terverifikasi
                @else
                    Terverifikasi
                @endif
            @elseif ($check_admin > 0)
                @if ($pengajuan_review_status[0]->$key == 0)
                    Tidak Terverifikasi
                @else
                    Terverifikasi
                @endif  
